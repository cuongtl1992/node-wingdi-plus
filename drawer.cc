...
void createDCWithoutDialog_v2(const Nan::FunctionCallbackInfo<Value>& args) {
	if (args.Length() < 1) {
		Nan::ThrowTypeError("wrong number of arguments");
		return;
	}
	...
	if (!args[0]->IsString()) {
		Nan::ThrowTypeError("wrong arguments type it must be string");
		return;
	}

	LPCSTR printerName = (LPCSTR)*String::Utf8Value(args[0]->ToString());
	...
	args.GetReturnValue().Set(Nan::New((int)hdcPrint));
}
...
void printImageFromBytes_v2(const Nan::FunctionCallbackInfo<Value>& args) {
	if (args.Length() < 2) {
		Nan::ThrowTypeError("wrong number of arguments");
		return;
	}
	...
	HDC hdcPrint = (HDC)args[0]->Int32Value();
	Local<String> imagePathArg = Local<String>::Cast(args[1]);
	String::Utf8Value utf8Value(imagePathArg);
	std::string imagePath(*utf8Value);
	UINT dimCount, frameCount;
	GUID* pDimIDs;
	GUID pageGuid;
	float width, height;
	DOCINFO di = { sizeof(DOCINFO), "Printing Image" };
	Graphics graphics(hdcPrint);
	int ret = StartDoc(hdcPrint, &di);
...
void Init(v8::Local<v8::Object> exports){
	...
	main();
	// v2
	exports->Set(Nan::New("createDCWithoutDialog_v2").ToLocalChecked(),
		Nan::New<v8::FunctionTemplate>(createDCWithoutDialog_v2)->GetFunction());
	exports->Set(Nan::New("printImageFromBytes_v2").ToLocalChecked(),
		Nan::New<v8::FunctionTemplate>(printImageFromBytes_v2)->GetFunction());
